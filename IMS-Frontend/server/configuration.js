...
class ConfigurationPage {
    ...
    // User info is now initialized by shared navbar.js
    ...
    setupEventListeners() {
        ...
        // Note: Navbar dropdown, change password, and logout are now handled by shared navbar.js
    }
    ...
    getFiltersForComponentType(componentType) {
        const filterConfigs = {
            'cpu': {
                ...
                threadCount: { type: 'range', label: 'THREAD COUNT', min: 1, max: 128, step: 1, unit: '' },
                coreClock: { type: 'range', label: 'PERFORMANCE CORE CLOCK', min: 1.1, max: 4.7, step: 0.1, unit: ' GHz' },
                l2Cache: { type: 'range', label: 'L2 CACHE', min: 0.25, max: 40, step: 0.25, unit: ' MB' },
                l3Cache: { type: 'range', label: 'L3 CACHE', min: 1, max: 200, step: 1, unit: ' MB' }
            },
            'ram': {
                manufacturer: { type: 'radio', label: 'MANUFACTURER', options: ['All', 'Corsair', 'G.Skill', 'Kingston', 'Crucial'] },
            ...
            'motherboard': {
                manufacturer: { type: 'radio', label: 'MANUFACTURER', options: ['All', 'ASUS', 'MSI', 'Gigabyte', 'ASRock', 'Supermicro'] },
                socket: { type: 'radio', label: 'SOCKET', options: ['All', 'LGA1700', 'AM5', 'AM4', 'LGA1200'] },
                formFactor: { type: 'radio', label: 'FORM FACTOR', options: ['All', 'ATX', 'Micro ATX', 'Mini ITX', 'E-ATX'] },
                chipset: { type: 'radio', label: 'CHIPSET', options: ['All', 'Z790', 'B760', 'X670', 'B650'] }
            },
            'storage': {
            ...
            'nic': {
                manufacturer: { type: 'radio', label: 'MANUFACTURER', options: ['All', 'Intel', 'Broadcom', 'Mellanox', 'Aquantia'] },
            ...
            'caddy': {
                manufacturer: { type: 'radio', label: 'MANUFACTURER', options: ['All', 'Dell', 'HPE', 'Supermicro'] },
                formFactor: { type: 'radio', label: 'SIZE', options: ['All', '2.5"', '3.5"'] },
                interface: { type: 'radio', label: 'INTERFACE', options: ['All', 'SATA', 'SAS', 'NVMe'] },
                hotSwap: { type: 'radio', label: 'HOT SWAP', options: ['All', 'Yes', 'No'] }
            },
            'pciecard': {
                ...
                memory: { type: 'range', label: 'MEMORY (GB)', min: 4, max: 48, step: 4, unit: ' GB' }
            },
            'hbacard': {
                manufacturer: { type: 'radio', label: 'MANUFACTURER', options: ['All', 'Broadcom', 'Adaptec', 'LSI'] },
                interface: { type: 'radio', label: 'INTERFACE', options: ['All', 'PCIe 3.0', 'PCIe 4.0'] },
                ports: { type: 'range', label: 'PORTS', min: 4, max: 16, step: 4, unit: '' },
    ...
    /**
     * Render filters dynamically based on component type
     */
        ...
        this.filteredComponents = this.components.filter(component => {
            ...
            for (const [key, config] of Object.entries(filterConfig)) {
                if (config.type === 'radio') {
                    ...
                    const rangeInput = document.getElementById(`${key}Range`);
                    if (rangeInput) {
                        const minValue = parseFloat(rangeInput.value);
                        const componentValue = parseFloat(this.formatValue(component[key]));

                        if (!isNaN(componentValue) && componentValue < minValue) {
                            return false;
                        }
                    }
                }
        ...
        switch (type) {
            case 'cpu':
                return `
                    ...
                    <div class="component-spec">${this.formatValue(component.graphics)}</div>
                `;
            case 'ram':
                return `
                    <div class="component-spec">${this.formatValue(component.capacity)}</div>
                    <div class="component-spec">${this.formatValue(component.speed) !== 'N/A' ? this.formatValue(component.speed) + ' MHz' : 'N/A'}</div>
                    <div class="component-spec">${this.formatValue(component.type)}</div>
                    <div class="component-spec">${this.formatValue(component.cas)}</div>
                    <div class="component-spec">${this.formatValue(component.voltage) !== 'N/A' ? this.formatValue(component.voltage) + ' V' : 'N/A'}</div>
            ...
            case 'motherboard':
                return `
                    ...
                    <div class="component-spec">${this.formatValue(component.pcieSlots)}</div>
                    <div class="component-spec">${this.formatValue(component.sataPorts)}</div>
                `;
            case 'storage':
                return `
                    <div class="component-spec">${this.formatValue(component.capacity)}</div>
                    ...
                    <div class="component-spec">${this.formatValue(component.readSpeed)}</div>
                    <div class="component-spec">${this.formatValue(component.writeSpeed)}</div>
                    <div class="component-spec">${this.formatValue(component.formFactor)}</div>
                `;
            case 'nic':
                return `
                    ...
                    <div class="component-spec">${this.formatValue(component.features)}</div>
                `;
            case 'chassis':
                return `
                    <div class="component-spec">${this.formatValue(component.formFactor)}</div>
                    <div class="component-spec">${this.formatValue(component.maxDrives)}</div>
                    <div class="component-spec">${this.formatValue(component.maxRAM)}</div>
                    <div class="component-spec">${this.formatValue(component.powerSupply)}</div>
                    <div class="component-spec">${this.formatValue(component.dimensions)}</div>
                    <div class="component-spec">${this.formatValue(component.weight)}</div>
                `;
            case 'caddy':
                return `
                    <div class="component-spec">${this.formatValue(component.formFactor)}</div>
                    ...
                    <div class="component-spec">${this.formatValue(component.capacity)}</div>
                    <div class="component-spec">${this.formatValue(component.speed)}</div>
                    <div class="component-spec">${this.formatValue(component.hotSwap)}</div>
                    <div class="component-spec">${this.formatValue(component.compatibility)}</div>
                `;
            case 'pciecard':
                return `
                    <div class="component-spec">${this.formatValue(component.interface)}</div>
                    <div class="component-spec">${this.formatValue(component.memory)}</div>
                    <div class="component-spec">${this.formatValue(component.cores)}</div>
                    <div class="component-spec">${this.formatValue(component.baseClock)}</div>
                    <div class="component-spec">${this.formatValue(component.boostClock)}</div>
                    <div class="component-spec">${this.formatValue(component.tdp)}</div>
                `;
            case 'hbacard':
                return `
                    ...
                    <div class="component-spec">${this.formatValue(component.interface)}</div>
                    <div class="component-spec">${this.formatValue(component.ports)}</div>
                    <div class="component-spec">${this.formatValue(component.controller)}</div>
                    <div class="component-spec">${this.formatValue(component.dataRate)}</div>
                    <div class="component-spec">${this.formatValue(component.features)}</div>
                `;
            default:
                return `
